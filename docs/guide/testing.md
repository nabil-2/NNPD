...
| Pipeline | `test_pipeline.py` | Four-prior sampling, training and inference; model reuse after adding metrics; learning-rate sweeps reusing data; relocation of an output root; train-then-analyze without implicit training; recovery from a failed stage; analytic oracle and a single prior factor; mean/standard-deviation parameter sets; plots and result readers; feasibility checks; latest-settings-win cleanup that keeps the cache of the last three settings; saved settings reload; failed launches delete nothing; a separate non-Gaussian experiment. |
| CLI | `test_cli.py` | `plan` without side effects; `train`, `analyze`, `verify`; the byte-for-byte settings copy and rerunning it. |
...
pairs, and the one-million-sample reweighting showcase. Its cost is dominated by
inference: up to about 2.5·10⁸ model evaluations per prior, reported by the plan as
