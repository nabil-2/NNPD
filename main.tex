...
- combine sec 2 and 3 / problem setup/background and subsections
- one section: gaussian examples
- one section: physical examples
...
\end{align*}
should hold. This implies that if $\mathbb{E} [r(x|\theta_0)]=1$, then $r(x|\theta_0) = p(x|\theta_0) / p(x) = p(\theta_0 | x) / p(\theta_0)$ is plausible. Thus a value of $\mathbb{E} [r(x|\theta_0)]=1$ that significally deviates from $1$ shows that the classifier is not ideal or any other conditions required for the likelihood ratio trick does not hold. Figure \ref{fig:ratio_violins} shows values of $\mathbb{E}[r(x|\theta_0)]$ for different parameter values and different classifiers trained on different prior distributions.

\begin{figure}[ht]
    \centering
    \includesvg[width=0.8\linewidth]{figs/config_005/verifications/ratio_violins.svg}
    ...
    \label{fig:ratio_violins}
\end{figure}

A more rigorous test of the likelihood ratio trick conditions is to reweight data.
For it to hold, it means that $f_\theta(x)/(1-f_\theta(x)) = \frac{p(\theta,x)}{p(x)p(\theta)}$ is true for any value of $\theta$ within the considered parameter space.
Assuming that this is fulfilled, reweighting data drawn from $p(x|\theta_0)$ to $p(x|\theta_1)$ is done as follows:
The likelihood ratio is calculated as $r(x|\theta) = \frac{p(\theta,x)}{p(x)p(\theta)} = \frac{p(x|\theta)}{p(x)}$ and the ratio of likelihood ratios $R_{01}(x)$ as
\begin{align*}
    R_{01}(x) = r(x|\theta_1)/r(x|\theta_0) = \frac{p(x|\theta_1)}{p(x|\theta_0)}
\end{align*}.
From there data is drawn from $p(x|\theta_0)$ and reweighted to $p(x|\theta_1) = R_{01}(x) p(x|\theta_0)$, which is then compared to data drawn directly from $p(x|\theta_1)$.
If the reweighted distribution closely resembles the original data distribution, this shows that the likelihood ratio trick conditions hold.
