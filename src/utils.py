...
def uniform(x, min_val=0, max_val=10):
    return 1 / (max_val - min_val) if min_val <= x <= max_val else 0.0
...
def gamma(x, shape=2, scale=2):
    if gamma_func is None:
        raise ImportError("scipy.special.gamma is unavailable in this environment.")
    return (
        (x ** (shape - 1) * np.exp(-x / scale)) / (scale**shape * gamma_func(shape))
        if x >= 0
        else 0.0
    )
...
def get_data(prior, config, show_output=True):
    ...
    parameters_all = sample_from_pdf(
        prior,
        3 * n_class,
        data_config["parameter_range"][0],
        data_config["parameter_range"][1],
    )
...
def get_filepath(filename, config, root="figs"):
    ...
    used_numbers = []
    for directory in root_path.glob("config_*"):
